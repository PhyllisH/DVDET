...
# GaussianSmooth_communication_rates_r3 = [1.339044, 0.150497034, 0.0969433, 0.0692113, 0.046438, 0.02628, 0.0195102, 0.0140386, 0.0089689, 0]

# GaussianSmooth_Where2comm_r3 = [
#     [0.6728657702736681,  0.6656237607928538, 0.6565800220121405, 0.6493068051646531,  0.6406852700159512,  0.636003145448309, 0.6354575265301962, 0.6332479868963521, 0.6191554409265326, 0.5981931055077964],
#     [0.4714416296371197,  0.4708731959590588, 0.4603811595313383, 0.45230290815005886,  0.4467870801831065,  0.4406921826770181, 0.43684378054409245, 0.43374600494238374, 0.42152438286383237, 0.4011335836871555],
#     [0.19073043099286743,  0.1842017285585523, 0.18227099165915386, 0.1802208449553001, 0.1777258421844148, 0.17146586873462594, 0.17117463577456757, 0.16836500147003142, 0.16091460671807545, 0.15361522854572895]
# ]
...
for i, metric in enumerate(metrics[:3]):
    ...
    plt.text(max_volume-3, where2comm[i]* 100-3, 'Where2comm'.format(max_volume,where2comm[i]* 100), ha='center', va='bottom', fontsize=fontsize)
    plt.scatter(0, No_Message[i]* 100, label='NoCollaboration', linewidth=3, c='mediumpurple', s=point_size)
    plt.text(8, No_Message[i]* 100, 'NoCollaboration'.format(0,No_Message[i]* 100), ha='center', va='bottom', fontsize=fontsize)
    plt.scatter(max_volume+math.log2(3), V2V[i]* 100, label='V2V', linewidth=3, c='orange', s=point_size)
    plt.text(max_volume+math.log2(3)-1, V2V[i]* 100+0.2, 'V2V'.format(max_volume+math.log2(3),V2V[i]* 100), ha='center', va='bottom', fontsize=fontsize)
    plt.scatter(max_volume, DiscoNet[i]* 100, label='DiscoNet', linewidth=3, c='olivedrab', s=point_size)
    plt.text(max_volume-2, DiscoNet[i]* 100-3, 'DiscoNet'.format(max_volume,DiscoNet[i]* 100), ha='center', va='bottom', fontsize=fontsize)
...
for i, metric in enumerate(metrics[:3]):
    ...
    multi_agent_withthre_results = [x * 100 for x in multi_agent_withthre_results[i]]
    communication_volume_bytes = [math.log2(communication_volume * 32 // 8 * r) for r in communication_rates[:-1]] + [0]
    plt.plot(communication_volume_bytes[:-1], multi_agent_withthre_results[:-1], label='Round3', linewidth=3)
    communication_rates = [1/x for x in communication_rates[:-1]] + [0]
    ...
    multi_agent_withthre_results = [x * 100 for x in multi_agent_withthre_results[i]]
    communication_volume_bytes = [math.log2(communication_volume * 32 // 8 * r) for r in communication_rates[:-1]] + [0]
    plt.plot(communication_volume_bytes[:-1], multi_agent_withthre_results[:-1], label='Round2', linewidth=3)
    communication_rates = [1/x for x in communication_rates[:-1]] + [0]
    ...
    multi_agent_withthre_results = [x * 100 for x in multi_agent_withthre_results[i]]
    communication_volume_bytes = [math.log2(communication_volume * 32 // 8 * r) for r in communication_rates[:-1]] + [0]
    plt.plot(communication_volume_bytes[:-1], multi_agent_withthre_results[:-1], label='Round1', linewidth=3, c='seagreen')
    communication_rates = [1/x for x in communication_rates[:-1]] + [0]
