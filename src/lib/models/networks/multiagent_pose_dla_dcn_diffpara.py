...
class DLASeg(nn.Module):
            ...
            for c_layer in self.trans_layer:
                for round_id in range(round):
                    if c_layer >= 0:
                        ...
                        dropout1 = nn.Dropout(dropout)
                        dropout2 = nn.Dropout(dropout)
                        self.__setattr__('cross_attn'+str(round_id), cross_attn)
                        self.__setattr__('linear1_'+str(round_id), linear1)
                        self.__setattr__('linear2_'+str(round_id), linear2)
                        self.__setattr__('norm1_'+str(round_id), norm1)
                        self.__setattr__('norm2_'+str(round_id), norm2)
                        self.__setattr__('dropout0_'+str(round_id), dropout0)
                        self.__setattr__('dropout1_'+str(round_id), dropout1)
                        self.__setattr__('dropout2_'+str(round_id), dropout2)
    ...
    def QUALITYMAP_MESSAGE(self, x, shift_mats, round, comm_thre=0.1, fusion='context', compress_flag=False):
        ...
        def _transformer_message_fusing(query_feat, val_feat, quality_map, round_id):
        ...
        def _partial_graph(max_edge_num=7):
            ...
            while True:
                ...
                if len(adj_mat.sum(axis=0).nonzero(0)[0]) == num_agents:
                    break
        ...
        for round_id in range(round):
            ...
            if prev_confidence_maps is None:
                val_feats[0], communication_mask, communication_rate, _ = _communication_graph_learning(val_feats[0], quality_maps[0], confidence_maps, round_id, thre=comm_thre)
            else:
                val_feats[0], communication_mask, communication_rate, _ = _communication_graph_learning(val_feats[0], quality_maps[0], prev_confidence_maps, round_id, thre=comm_thre)
            ...
            for i, c_layer in enumerate(self.trans_layer):
                ...
                feat_fuse, weight_mat = _transformer_message_fusing(feat_map, val_feat, quality_map, round_id)
