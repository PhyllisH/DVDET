...
class DLASeg(nn.Module):
    ...
    def QUALITYMAP_MESSAGE(self, x, shift_mats, round, comm_thre=0.1, fusion='context', compress_flag=False):
        ...
        def _communication_graph_learning(val_feats, quality_maps, confidence_maps, round_id=0, thre=0.03):
            ...
            communication_mask, communication_maps_topk = _bandwidthAware_comm_mask(communication_maps, B=thre)
            ...
            return val_feats_aftercom, communication_mask, communication_rate, communication_maps_topk_nodiag
        ...
        for round_id in range(round):
            ...
            if fusion == 'egodcntransformer':
                ...
                picked_agent = np.random.choice(num_agents, size=1, replace=False)
                # picked_agent = np.arange(num_agents)
                val_feat_with_pos = torch.cat([val_feats[0][:,:,picked_agent], pos_maps[0][:,:,picked_agent]], dim=-3) # (b, k_agents,num_picked_agent, c+2, h, w)
                val_feat_with_pos = val_feat_with_pos.flatten(0,2)  # (b, k_agents,num_picked_agent, c+2, h, w)
                ego_feat_with_pos = torch.cat([x[0][:,picked_agent], pos_map[:,picked_agent]], dim=-3)  # (b, num_picked_agent, c+2, h, w)
                ego_feat_with_pos = ego_feat_with_pos.unsqueeze(1).expand(-1, num_agents, -1, -1, -1, -1) # (b, k_agents,num_picked_agent, c+2, h, w)
                ego_feat_with_pos = ego_feat_with_pos.flatten(0,2)
                val_feat_with_pos_DW = eval('self.DC'+str(self.trans_layer[0]))(ego_feat_with_pos.contiguous(), val_feat_with_pos.contiguous())
                val_feat_with_pos = (val_feat_with_pos[:,:c] + val_feat_with_pos_DW)*0.5
                val_feat_with_pos = val_feat_with_pos.view(b, num_agents, len(picked_agent), c, h, w).contiguous()
                val_feats[0][:,:,picked_agent] = val_feat_with_pos
                # val_feats[0] = val_feat_with_pos
            
            if prev_confidence_maps is None:
                val_feats[0], communication_mask, communication_rate, quality_maps[0] = _communication_graph_learning(val_feats[0], quality_maps[0], confidence_maps, round_id, thre=comm_thre)
            else:
                val_feats[0], communication_mask, communication_rate, quality_maps[0] = _communication_graph_learning(val_feats[0], quality_maps[0], prev_confidence_maps, round_id, thre=comm_thre)
                ...
                for i in range(b):
                    ...
                    for j in range(q_agents):
                        ...
                        for k in range(k_agents):
                            ...
                            axes[2,k].imshow((cur_images[i,k,j].detach().cpu().numpy().transpose(1,2,0) * 255.).astype('uint8')) # (b, q_agents, h, w)
                            axes[2,k].set_xticks([])
                            axes[2,k].set_yticks([])
                        fig.tight_layout()
                        plt.savefig(os.path.join(save_dir, '{}.png'.format(j)))
