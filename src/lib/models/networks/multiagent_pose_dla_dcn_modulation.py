            ...
            for c_layer in self.trans_layer:
                if c_layer >= 0:
                    ...
                    weight_net = nn.Sequential(
                                    nn.Conv2d(d_model*2, 32,
                                        kernel_size=1, padding=0, bias=True),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(32, 1,
                                        kernel_size=1, padding=0, bias=True))

                    self.add_module('weight_net', weight_net)
    ...
    def QUALITYMAP_MESSAGE(self, x, shift_mats, round, comm_thre=0.1, fusion='context', compress_flag=False):
        ...
        for round_id in range(round):
            ...
            for i, c_layer in enumerate(self.trans_layer):
                ...
                # feats = torch.cat([x[c_layer], feat_fuse], dim=2) # (b, n, 2*c, h, w)
                # feats = feats.flatten(0,1)
                # weight = self.weight_net(feats)
                # weight = weight.view(b, num_agents, 1, h, w)
                # weight = weight.sigmoid()
                # x[c_layer] = feat_fuse * weight + x[c_layer] * (1-weight)
