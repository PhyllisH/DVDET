...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
                 ...
                 feat_mode='inter', feat_shape=[192, 352], round=1, compress_flag=False, comm_thre=0.1):
        ...
        if self.message_mode in ['When2com']:
            ...
            for c_layer in self.trans_layer:
                if c_layer >= 0:
                    d_model = 64*2**c_layer
                    cross_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
                    # cross_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout, kdim=d_model+1, vdim=d_model)
                    # Implementation of Feedforward model
                    linear1 = nn.Linear(d_model, d_model)
                    linear2 = nn.Linear(d_model, d_model)
                    ...
                    if 'EgoDCN' in self.message_mode:
                        ...
                        k_size = 5
                        n_points = 9
                        conv_offset_mask = nn.Conv2d((d_model+2)*2, n_points*2, kernel_size=k_size, stride=1, padding=(k_size-1)//2, bias=True)
                        self.__setattr__('conv_offset_mask'+str(c_layer), conv_offset_mask)
                        self.init_offset(c_layer)
    ...
    def init_offset(self, c_layer):
        eval('self.conv_offset_mask'+str(c_layer)).weight.data.zero_()
        eval('self.conv_offset_mask'+str(c_layer)).bias.data.zero_()
    ...
    def QUALITYMAP_MESSAGE(self, x, shift_mats, round, comm_thre=0.1, fusion='context', compress_flag=False):
        ...
        def _noquery_maxcontext_message_fusing(query_feat, val_feat, quality_map):
            b, num_agents, c, h, w = query_feat.shape
            # query_feat = query_feat.unsqueeze(1).expand(-1, num_agents, -1, -1, -1, -1).contiguous()   # (b*num_agents, c, h, w) --> (b, k_agents, q_agents, c, h, w)
            # context_feat_mask = torch.where(val_feat>0, torch.ones_like(val_feat).to(val_feat.device), torch.zeros_like(val_feat).to(val_feat.device))
            # context_feat_mask = context_feat_mask.sum(dim=1).unsqueeze(1).expand(-1, num_agents, -1, -1, -1, -1).contiguous() - context_feat_mask
            ...
            context_feat = val_feat.max(dim=1)[0].unsqueeze(1).expand(-1, num_agents, -1, -1, -1, -1) # (b, q_agents, c, h, w)
        ...
        def _query_maxcontext_message_fusing(query_feat, val_feat, quality_map):
            b, num_agents, c, h, w = query_feat.shape
            query_feat = query_feat.unsqueeze(1).expand(-1, num_agents, -1, -1, -1, -1).contiguous()   # (b*num_agents, c, h, w) --> (b, k_agents, q_agents, c, h, w)
            # context_feat_mask = torch.where(val_feat>0, torch.ones_like(val_feat).to(val_feat.device), torch.zeros_like(val_feat).to(val_feat.device))
            # context_feat_mask = context_feat_mask.sum(dim=1).unsqueeze(1).expand(-1, num_agents, -1, -1, -1, -1).contiguous() - context_feat_mask
            ...
            context_feat = val_feat.max(dim=1)[0].unsqueeze(1).expand(-1, num_agents, -1, -1, -1, -1)
        ...
        def _transformer_message_fusing(query_feat, val_feat, quality_map):
            ...
            # max_feat = val_feat.max(dim=1)[0]
            # max_feat = self.add_pe_map(max_feat)
            # src = max_feat.permute(0,1,3,4,2).contiguous().view(b*num_agents*h*w,c).contiguous().unsqueeze(0)
            ...
            weight_mat = weight_mat.view(b, num_agents, h, w, num_agents).contiguous().permute(0, 1, 4, 2, 3).contiguous().unsqueeze(3)
            return feat_fuse, weight_mat
        
        def _difftransformer_message_fusing(query_feat, val_feat, quality_map):
            query_feat = self.add_pe_map(query_feat)
            b, num_agents, c, h, w = query_feat.size()
        ...
        def _cattransformer_message_fusing(query_feat, val_feat, quality_map, confidence_map):
            query_feat = self.add_pe_map(query_feat)
            b, num_agents, c, h, w = query_feat.size()            

            src = query_feat.permute(0,1,3,4,2).contiguous().view(b*num_agents*h*w,c).contiguous().unsqueeze(0)    # (1, b*num_agents*h*w, c)
            tgt = torch.cat([val_feat, quality_map], dim=-3)
            tgt = tgt.permute(1,0,2,4,5,3).contiguous().view(num_agents, b*num_agents*h*w,c+1).contiguous()
            val = val_feat.permute(1,0,2,4,5,3).contiguous().view(num_agents, b*num_agents*h*w,c).contiguous()
            # quality_map = quality_map.permute(0,2,3,4,5,1).contiguous().view(b*num_agents*h*w, 1, num_agents).contiguous()
            # tgt = get_local_feat(tgt_feat.view(b*num_agents*num_agents, c, h, w).contiguous(), kernel_size=kernel_size, stride=stride)
            # tgt = tgt.contiguous().view(b, num_agents, num_agents, kernel_size**2, c, h, w).contiguous()
        ...
        def _weighttransformer_message_fusing(query_feat, val_feat, quality_map):
            query_feat = self.add_pe_map(query_feat)
            b, num_agents, c, h, w = query_feat.size()
            
            src = query_feat.permute(0,1,3,4,2).contiguous().view(b*num_agents*h*w,c).contiguous().unsqueeze(0)    # (1, b*num_agents*h*w, c)
            tgt = val_feat * quality_map
            tgt = tgt.permute(1,0,2,4,5,3).contiguous().view(num_agents, b*num_agents*h*w,c).contiguous()    # (num_agents, b*num_agents*h*w, c)
            val = val_feat.permute(1,0,2,4,5,3).contiguous().view(num_agents, b*num_agents*h*w,c).contiguous()
            # quality_map = quality_map.permute(0,2,3,4,5,1).contiguous().view(b*num_agents*h*w, 1, num_agents).contiguous()
            # tgt = get_local_feat(tgt_feat.view(b*num_agents*num_agents, c, h, w).contiguous(), kernel_size=kernel_size, stride=stride)
            # tgt = tgt.contiguous().view(b, num_agents, num_agents, kernel_size**2, c, h, w).contiguous()
        ...
        def _selfattn_accord_qmap_v1(val_feat, quality_map):
            def dis_map(h, w, y_c, x_c, grid=25, bin=False):
                mask = torch.zeros([h, w], dtype=torch.bool, device=x_c.device)
                not_mask = ~mask
                y_embed = not_mask.cumsum(0, dtype=torch.float32)
                x_embed = not_mask.cumsum(1, dtype=torch.float32)

                x_embed = x_embed.unsqueeze(0).expand(x_c.shape[0],-1,-1)
                y_embed = y_embed.unsqueeze(0).expand(x_c.shape[0],-1,-1)
                x_mask = torch.where((torch.abs(x_embed - x_c.unsqueeze(-1)) - grid)>0, torch.zeros_like(x_embed).to(x_embed.device), torch.ones_like(x_embed).to(x_embed.device))
                y_mask = torch.where((torch.abs(y_embed - y_c.unsqueeze(-1)) - grid)>0, torch.zeros_like(y_embed).to(y_embed.device), torch.ones_like(y_embed).to(y_embed.device))
                if bin:
                    prob = x_mask * y_mask
                else:
                    dis = (((x_embed - x_c.unsqueeze(-1))/x_c.unsqueeze(-1)) ** 2 + ((y_embed - y_c.unsqueeze(-1))/y_c.unsqueeze(-1)) ** 2).sqrt()
                    prob = (1 - dis.sigmoid())
                    # prob = prob * x_mask * y_mask
                    # min_prob = (1 - 1/(1 + math.exp(-grid*math.sqrt(2))))
                    # min_prob = torch.tensor(min_prob).to(x_c.device)
                    # prob = (prob - min_prob)/(prob.max()-min_prob)  # normalize
                return prob
            # confidence_map: b, num_agents, num_agents, 1, h, w
            # feat_map: b, num_agents, num_agents, c, h, w
            feat_maps_withpos = self.add_pe_map(val_feat)
            shape_len = len(feat_maps_withpos.shape)

            if len(feat_maps_withpos.shape) == 6:
                b, num_agents, num_agents, c, h, w = feat_maps_withpos.shape
                feat_maps = val_feat.view(b*num_agents*num_agents, c, h, w)
                feat_maps_withpos = feat_maps_withpos.view(b*num_agents*num_agents, c, h, w)
                quality_map = quality_map.view(b*num_agents*num_agents, h, w)
            else:
                b, num_agents, c, h, w = feat_maps_withpos.shape
                feat_maps = val_feat.view(b*num_agents, c, h, w)
                feat_maps_withpos = feat_maps_withpos.view(b*num_agents, c, h, w)
                quality_map = quality_map.view(b*num_agents, h, w)

            # Get the topk feature to be strengthened
            grid = 40 // 2
            max_len = 100
            local_len = 100
            flatten_quality_map = quality_map.flatten(1) # (b*num_agents*num_agents, h*w)
            flatten_feat_maps = feat_maps.flatten(2) # (b*num_agents*num_agents,c,h*w)
            flatten_feat_maps_withpos = feat_maps_withpos.flatten(2) # (b*num_agents*num_agents,c,h*w)
            _, indices = torch.topk(flatten_quality_map, max_len, dim=-1)

            topk_feats = flatten_feat_maps.gather(2, indices.unsqueeze(1).expand(-1,c,-1))    # (b*num_agents*num_agents,c,max_len)
            topk_feats_withpos = flatten_feat_maps_withpos.gather(2, indices.unsqueeze(1).expand(-1,c,-1))

            flatten_indices = indices.flatten(0)
            h_indices = (flatten_indices // w).unsqueeze(1)
            w_indices = (flatten_indices % w).unsqueeze(1)
            p_dis = dis_map(h, w, h_indices, w_indices, grid) # (b*num_agents*max_len, h, w)
            # p_dis = dis_map(h, w, h_indices, w_indices, grid, bin=True) # (b*num_agents*max_len, h, w)
            p_dis = p_dis.flatten(1)
            quality_map_withdis = flatten_quality_map.unsqueeze(1).expand(-1,max_len,-1).flatten(0,1) * p_dis # (b*num_agents*num_agents*max_len, h*w)

            _, local_indices = torch.topk(quality_map_withdis, local_len, dim=-1)   # (b*num_agents*num_agents*max_len,local_len)
            # _, local_indices = torch.topk(p_dis, local_len, dim=-1)
            local_indices = local_indices.view(-1, max_len, local_len)
            local_topk_feats = []
            local_topk_feats_withpos = []
            for i in range(max_len):
                cur_topk_local_feats = flatten_feat_maps.gather(2, local_indices[:,i].unsqueeze(1).expand(-1,c,-1))    # (b*num_agents*num_agents,c,local_len)
                cur_topk_local_feats_withpos = flatten_feat_maps_withpos.gather(2, local_indices[:,i].unsqueeze(1).expand(-1,c,-1))
                local_topk_feats.append(cur_topk_local_feats.unsqueeze(1))
                local_topk_feats_withpos.append(cur_topk_local_feats_withpos.unsqueeze(1))
            local_topk_feats = torch.cat(local_topk_feats, dim=1)   # (b*num*num, max_len, c, local_len)
            local_topk_feats_withpos = torch.cat(local_topk_feats_withpos, dim=1)   # (b*num*num, max_len, c, local_len)

            src = topk_feats.permute(0,2,1).contiguous().view(-1,c).unsqueeze(0)   # (1, b*num_agents*num_agents*h*w*max_len, c)
            val = local_topk_feats.flatten(0,1).permute(2,0,1).contiguous() # (local_len, b*num_agents*num_agents*h*w*max_len, c)
            tgt = local_topk_feats_withpos.flatten(0,1).permute(2,0,1).contiguous()

            src2 = eval('self.self_attn'+str(c_layer))(src, tgt, value=tgt, attn_mask=None, key_padding_mask=None)[0]
            src = src + eval('self.dropout3_'+str(c_layer))(src2)
            src = eval('self.norm3_'+str(c_layer))(src) # (1, b*num_agents*num_agents*h*w*max_len, c)

            # Push back to feature map
            src = src.permute(1, 2, 0).view(-1, max_len, c).permute(0,2,1).contiguous() # (b*num_agents*num_agents, max_len, c)
            val_feat = flatten_feat_maps.clone().scatter_(2, indices.unsqueeze(1).expand(-1,c,-1), src) # (b*num_agents*num_agents,c,h*w)
            if shape_len == 6:
                val_feat = val_feat.view(b, num_agents, num_agents, c, h, w).contiguous()
                quality_map = quality_map.view(b, num_agents, num_agents, 1, h, w).contiguous()
            else:
                val_feat = val_feat.view(b, num_agents, c, h, w).contiguous()
                quality_map = quality_map.view(b, num_agents, 1, h, w).contiguous()
            return val_feat, quality_map

        def _selfattn_accord_qmap_v0(val_feat, quality_map):
            # confidence_map: b, num_agents, num_agents, 1, h, w
            # feat_map: b, num_agents, num_agents, c, h, w
            feat_maps_withpos = self.add_pe_map(val_feat)
            shape_len = len(feat_maps_withpos.shape)
            ...
            quality_map = quality_map.flatten(1) # (b*num_agents*num_agents, h*w)
            feat_maps = feat_maps.flatten(2) # (b*num_agents*num_agents,c,h*w)
            feat_maps_withpos = feat_maps_withpos.flatten(2) # (b*num_agents*num_agents,c,h*w)

            max_len = [len(qmap.nonzero()) for qmap in quality_map]
            max_len = min(max(max_len), 800)

            topk_v, indices = torch.topk(quality_map, max_len, dim=-1)  # topk_v: (b*num_agents*num_agents,max_len)
            indices = indices.unsqueeze(1).expand(-1,c,-1)
            topk_feats = feat_maps.gather(2, indices)    # (b*num_agents*num_agents,c,max_len)
            topk_feats_withpos = feat_maps_withpos.gather(2, indices)
            
            # Self Attention
            # quality_map mode 0
            # topk_feats_withpos = topk_feats_withpos * topk_v.unsqueeze(1)
            q = k = topk_feats_withpos.permute(2, 0, 1).contiguous() # (max_len, b*num_agents*num_agents, c)
            src = topk_feats.permute(2, 0, 1).contiguous()
            # quality_map mode 1
            # self_quality_map = topk_v.unsqueeze(1).expand(-1,max_len,-1)
            # src2 = eval('self.self_attn'+str(c_layer))(q, k, value=src, attn_mask=None, key_padding_mask=None, quality_map=self_quality_map)[0]
            src2 = eval('self.self_attn'+str(c_layer))(q, k, value=src, attn_mask=None, key_padding_mask=None)[0]
            src = src + eval('self.dropout3_'+str(c_layer))(src2)
            src = eval('self.norm3_'+str(c_layer))(src)

            # Push back to feature map
            src = src.permute(1, 2, 0) # (max_len, b*num_agents*num_agents, c)
            val_feat = feat_maps.clone().scatter_(2, indices, src) # (b*num_agents*num_agents,c,h*w)
            if shape_len == 6:
                val_feat = val_feat.view(b, num_agents, num_agents, c, h, w).contiguous()
                quality_map = quality_map.view(b, num_agents, num_agents, 1, h, w).contiguous()
            else:
        ...
        def _crossselftransformer_message_fusing(query_feat, val_feat, quality_map):
            # confidence_maps: b, num_agents, 1, h, w
            query_feat = self.add_pe_map(query_feat)
            b, num_agents, c, h, w = query_feat.size()
            ...
            # src2, cross_weight_mat = eval('self.cross_attn'+str(c_layer))(src, val, value=val, attn_mask=None, key_padding_mask=None, quality_map=quality_map)
            src2, cross_weight_mat = eval('self.cross_attn'+str(c_layer))(src, val, value=val, attn_mask=None, key_padding_mask=None)
            src = src + eval('self.dropout1_'+str(c_layer))(src2)
            src = eval('self.norm1_'+str(c_layer))(src)

            src = src.view(b, num_agents, h, w, c).contiguous().permute(0, 1, 4, 2, 3).contiguous()
            quality_map = quality_map.max(dim=1)[0] # (b, num_agent, 1, h, w)
            src, quality_map = _selfattn_accord_qmap_v0(src, quality_map)
            # src, quality_map = _selfattn_accord_qmap_v1(src, quality_map)
            src = src.view(b*num_agents, c, h, w).flatten(2).permute(2, 0, 1).contiguous()  # (b, num_agent, c, h, w)

            # src = src.view(b, num_agents, h, w, c).contiguous().permute(0, 1, 4, 2, 3).contiguous()
            # q = self.add_pe_map(src)
            # src = src.view(b*num_agents, c, h, w).flatten(2).permute(2, 0, 1).contiguous()
            # q = k = q.view(b*num_agents, c, h, w).flatten(2).permute(2, 0, 1).contiguous()
            # # self_quality_maps = cross_weight_mat.sum(dim=-1).view(b, num_agents, h, w).contiguous().view(b*num_agents, h, w).flatten(1).unsqueeze(1).expand(-1,h*w,-1)
            # # src2 = eval('self.self_attn'+str(c_layer))(q, k, value=src, attn_mask=None, key_padding_mask=None, quality_map=self_quality_maps)[0]
            # src2 = eval('self.self_attn'+str(c_layer))(q, k, value=src, attn_mask=None, key_padding_mask=None)[0]
            # src = src + eval('self.dropout3_'+str(c_layer))(src2)
            # src = eval('self.norm3_'+str(c_layer))(src)
            ...
            feat_fuse = src.view(b, num_agents, h, w, c).contiguous().permute(0, 1, 4, 2, 3).contiguous()
            weight_mat = cross_weight_mat.view(b, num_agents, h, w, num_agents).contiguous().permute(0, 1, 4, 2, 3).contiguous().unsqueeze(3)
            return feat_fuse, weight_mat
        
        def _selfcrosstransformer_message_fusing(query_feat, val_feat, quality_map):            
            # val_feat, quality_map = _selfattn_accord_qmap_v0(val_feat, quality_map)
            val_feat, quality_map = _selfattn_accord_qmap_v1(val_feat, quality_map)
        ...
        def _crosstransformer_message_fusing(query_feat, val_feat, quality_map, grid=25, max_len=200, local_len=200, bin=False):
            def dis_map(h, w, y_c, x_c, grid=25, bin=False):
                mask = torch.zeros([h, w], dtype=torch.bool, device=x_c.device)
                not_mask = ~mask
                y_embed = not_mask.cumsum(0, dtype=torch.float32)
            ...
            # feat_map: b, num_agents, num_agents, c, h, w
            feat_maps_withpos = self.add_pe_map(val_feat)
            b, num_agents, num_agents, c, h, w = val_feat.shape
            shape_len = len(feat_maps_withpos.shape)

            # Get query feature map (max_len of the max confidence map)
            # union_quality_map = quality_map.max(dim=1)[0] # (b, q_agents, h, w)
            union_quality_map = []
            for i in range(num_agents):
                union_quality_map.append((torch.cat([quality_map[:,:i,i],quality_map[:,i+1:,i]], dim=1).max(dim=1)[0] - quality_map[:,i,i]).unsqueeze(1))
            union_quality_map = torch.cat(union_quality_map, dim=1)
            union_quality_map = union_quality_map.flatten(0,1).flatten(1) # (b*q_agents, h*w)
            # union_feat_maps = val_feat.max(dim=1)[0] # (b, q_agents, c, h, w)
            union_feat_maps = self.add_pe_map(query_feat)
            
            topk_v, indices = torch.topk(union_quality_map, max_len, dim=-1)  # topk_v: (b*num_agents,max_len)

            # print('max map: ', topk_v[:,-1].mean())
            indices_padc = indices.unsqueeze(1).expand(-1,c,-1)
            
            union_feat_maps = union_feat_maps.flatten(0,1).flatten(2) # (b*num_agents,c,h*w)
            topk_union_feats = union_feat_maps.gather(2, indices_padc)    # (b*num_agents,c,max_len)

            # Get key feature map (local_len among local region of all the agents)
            flatten_indices = indices.flatten(0)    # (b*num_agents*max_len)
            h_indices = (flatten_indices // w).unsqueeze(1)
            w_indices = (flatten_indices % w).unsqueeze(1)
            p_dis = dis_map(h, w, h_indices, w_indices, grid=grid, bin=True) # (b*num_agents*max_len, h, w)
            
            # image = p_dis[0].detach().cpu() * 255.
            # image = kornia.tensor_to_image(image.byte())
            # cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            # cv2.imwrite('dis_img.png', image)
            # import ipdb; ipdb.set_trace()
            p_dis = p_dis.flatten(1)    # (b*num_agents*max_len, h, w)
            p_dis = p_dis.view(b, num_agents, max_len, h, w).unsqueeze(1).expand(-1,num_agents,-1,-1,-1,-1) # (b, num_agents, num_agents, max_len, h, w)
            if bin:
                p_dis = p_dis.permute(0,2,3,1,4,5).flatten(0,2).flatten(1)
                local_topk_v, local_indices = torch.topk(p_dis, local_len, dim=-1)
            else:
                quality_map_withdis = quality_map.expand(-1,-1,-1,max_len,-1,-1) * p_dis # (b, num_agents, num_agents, max_len, h, w)
                quality_map_withdis = quality_map_withdis.permute(0,2,3,1,4,5).flatten(0,2).flatten(1)  # (b*q_agents*max_len, k_agents*h*w)
                local_topk_v, local_indices = torch.topk(quality_map_withdis, local_len, dim=-1)   # (b*num_agents*max_len,local_len)
            # print('local: ', local_topk_v[:,-1].mean())
            local_indices = local_indices.view(-1, max_len, local_len)
            flatten_feat_maps = val_feat.permute(0,2,3,1,4,5).flatten(0,1).flatten(2)   # (b,q_agents,c,k_agents,h,w)
            flatten_feat_maps_withpos = feat_maps_withpos.permute(0,2,3,1,4,5).flatten(0,1).flatten(2)   # (b,q_agents,c,k_agents,h,w)
            local_topk_feats = []
            local_topk_feats_withpos = []
            for i in range(max_len):
                cur_topk_local_feats = flatten_feat_maps.gather(2, local_indices[:,i].unsqueeze(1).expand(-1,c,-1))    # (b*q_agents,c,local_len)
                cur_topk_local_feats_withpos = flatten_feat_maps_withpos.gather(2, local_indices[:,i].unsqueeze(1).expand(-1,c,-1))
                local_topk_feats.append(cur_topk_local_feats.unsqueeze(1))
                local_topk_feats_withpos.append(cur_topk_local_feats_withpos.unsqueeze(1))
            local_topk_feats = torch.cat(local_topk_feats, dim=1)   # (b*q_agents, max_len, c, local_len)
            local_topk_feats_withpos = torch.cat(local_topk_feats_withpos, dim=1)   # (b*q_agents, max_len, c, local_len)
            
            src = topk_union_feats.permute(0,2,1).contiguous().flatten(0,1).unsqueeze(0)   # (1, b*q_agents*max_len, c)
            val = local_topk_feats.flatten(0,1).permute(2,0,1).contiguous() # (local_len, b*num_agents*max_len, c)
            tgt = local_topk_feats_withpos.flatten(0,1).permute(2,0,1).contiguous() # (local_len, b*num_agents*max_len, c)
            ...
            src2, weight_mat = eval('self.self_attn'+str(c_layer))(src, tgt, value=val, attn_mask=None, key_padding_mask=None)
            src = src + eval('self.dropout1_'+str(c_layer))(src2)
            src = eval('self.norm3_'+str(c_layer))(src)
            src2 = eval('self.linear2_'+str(c_layer))(eval('self.dropout0_'+str(c_layer))(F.relu(eval('self.linear1_'+str(c_layer))(src))))
            src = src + eval('self.dropout2_'+str(c_layer))(src2)
            src = eval('self.norm4_'+str(c_layer))(src)

            # Push back to feature map
            src = src.permute(1, 2, 0).view(-1, max_len, c).permute(0,2,1).contiguous() # (b*num_agents, c, max_len)
            fused_feat_maps = union_feat_maps.clone().scatter_(2, indices_padc, src) # (b*num_agents,c,h*w)
            # fused_feat_maps = union_feat_maps # (b*num_agents,c,h*w)
            # fused_feat_maps = query_feat.flatten(0,1).flatten(2).clone().scatter_(2, indices_padc, src)
            # fused_feat_maps = query_feat.flatten(0,1).flatten(2).clone().scatter_(2, indices_padc, union_feat_maps.gather(2, indices_padc) )
            fused_feat_maps = fused_feat_maps.view(b, num_agents, c, h, w).contiguous()
            
            # weight_mat = weight_mat.view(b, num_agents, max_len, local_len).contiguous().contiguous().unsqueeze(3)
            ############################ TOPK Quality Map Visualization ######################
            ...
            # saved_topk_union_weight_map = torch.zeros_like(p_dis).to(union_quality_map.device)  # (b, num_agents, num_agents, max_len, h, w)
            # weight_mat = weight_mat.view(b, num_agents, max_len, local_len).contiguous().view(b, num_agents, max_len, )contiguous().unsqueeze(3)
            
            #####################################################################

            return fused_feat_maps, quality_map.max(dim=1)[0]
        


        def _rescrosstransformer_message_fusing(query_feat, val_feat, quality_map):
            query_feat = self.add_pe_map(query_feat)
            b, num_agents, c, h, w = query_feat.size()
            
            src = query_feat.permute(0,1,3,4,2).contiguous().view(b*num_agents*h*w,c).contiguous().unsqueeze(0)    # (1, b*num_agents*h*w, c)
            val = val_feat.permute(1,0,2,4,5,3).contiguous().view(num_agents, b*num_agents*h*w,c).contiguous()
            # quality_map = quality_map.clone().permute(0,2,3,4,5,1).contiguous().view(b*num_agents*h*w, 1, num_agents).contiguous()
            
            # src = query_feat.clone().permute(0,1,3,4,2).contiguous().view(b*num_agents*h*w,c).contiguous().unsqueeze(0)    # (1, b*num_agents*h*w, c)
            # src2, cross_weight_mat = eval('self.cross_attn'+str(c_layer))(src, val, value=val, attn_mask=None, key_padding_mask=None, quality_map=quality_map)
            src2, cross_weight_mat = eval('self.cross_attn'+str(c_layer))(src, val, value=val, attn_mask=None, key_padding_mask=None)
            src2 = src + eval('self.dropout1_'+str(c_layer))(src2)
            src2 = eval('self.norm1_'+str(c_layer))(src2)

            src3 = eval('self.linear2_'+str(c_layer))(eval('self.dropout0_'+str(c_layer))(F.relu(eval('self.linear1_'+str(c_layer))(src2))))
            src3 = src2 + eval('self.dropout2_'+str(c_layer))(src3)
            src3 = eval('self.norm2_'+str(c_layer))(src3)
            feat_fuse = src3.view(b, num_agents, h, w, c).contiguous().permute(0, 1, 4, 2, 3).contiguous()
            weight_mat = cross_weight_mat.view(b, num_agents, h, w, num_agents).contiguous().permute(0, 4, 1, 2, 3).contiguous().unsqueeze(3)
            
            top_feat_fuse, top_weight_mat = _crosstransformer_message_fusing(feat_fuse, val_feat, quality_map, grid=3, max_len=100, local_len=100, bin=True)
            # top_feat_fuse, top_weight_mat = _crosstransformer_message_fusing(query_feat, val_feat, quality_map, grid=25, max_len=100, local_len=100, bin=False)
            final_feat_fuse = (top_feat_fuse + feat_fuse) * 0.5
            # return [feat_fuse, top_feat_fuse, final_feat_fuse], quality_map
            # return feat_fuse, final_feat_fuse, weight_mat
            return feat_fuse, feat_fuse, weight_mat
            # return top_feat_fuse, weight_mat
            # return feat_fuse, weight_mat
        
        def _cascrosstransformer_message_fusing(query_feat, val_feat, quality_map):
            query_feat = self.add_pe_map(query_feat)
            b, num_agents, c, h, w = query_feat.size()
            ...
            src = src2.view(b, num_agents, h, w, c).contiguous().permute(0, 1, 4, 2, 3).contiguous()
            feat_fuse = src2.clone().detach()
            src2, top_weight_mat = _crosstransformer_message_fusing(src, val_feat, quality_map, grid=10, max_len=100, local_len=100, bin=True)
            
            # top_feat_fuse, top_weight_mat = _crosstransformer_message_fusing(query_feat, val_feat, quality_map, grid=25, max_len=100, local_len=100, bin=False)
            final_feat_fuse = src2.view(b, num_agents, h, w, c).contiguous().permute(0, 1, 4, 2, 3).contiguous()
            weight_mat = cross_weight_mat.view(b, num_agents, h, w, num_agents).contiguous().permute(0, 1, 4, 2, 3).contiguous().unsqueeze(3)

            # return final_feat_fuse, weight_mat
            return [feat_fuse, final_feat_fuse, final_feat_fuse], top_weight_mat
        
        def _bevtransformer_message_fusing(query_feat, val_feat, quality_map):
        ...
        def _communication_graph_learning(val_feats, quality_maps, confidence_maps, round_id=0, thre=0.03):
            ...
            return val_feats_aftercom, communication_mask, communication_rate
        ...
        for round_id in range(round):
            ...
            val_feats = self.get_colla_feats(x, shift_mats, self.trans_layer, with_pos=True) # (b, k_agents, q_agents, c, h, w)
            if fusion == 'egodcntransformer':
                pos_h = torch.arange(h).unsqueeze(-1).expand(-1,w).unsqueeze(0) / h
                pos_w = torch.arange(w).unsqueeze(0).expand(h,-1).unsqueeze(0) / w - 0.5
                pos_map = torch.cat([pos_h, pos_w], dim=0).to(x[0].device) # (2, h, w)
                pos_map = pos_map.unsqueeze(0).expand(b,-1,-1,-1).unsqueeze(1).expand(-1,num_agents,-1,-1,-1) # (b, num_agents, 2, h, w)
                pos_map_list = [0,0,0,0]
                pos_map_list[self.trans_layer[0]] = pos_map
                pos_maps = self.get_colla_feats(pos_map_list, shift_mats, self.trans_layer, with_pos=False) # (b, k_agents, q_agents, 2, h, w)

                val_feat_with_pos = torch.cat([val_feats[0], pos_maps[0]], dim=-3) # (b, k_agents, q_agents, c+2, h, w)
                val_feat_with_pos = val_feat_with_pos.flatten(0,2)  # (b, k_agents, q_agents, c+2, h, w)
                ego_feat_with_pos = torch.cat([x[0], pos_map], dim=-3)  # (b, q_agents, c+2, h, w)
                ego_feat_with_pos = ego_feat_with_pos.unsqueeze(1).expand(-1, num_agents, -1, -1, -1, -1) # (b, k_agents, q_agents, c+2, h, w)
                ego_feat_with_pos = ego_feat_with_pos.flatten(0,2)
                val_feat_with_pos_DW = eval('self.DC'+str(self.trans_layer[0]))(ego_feat_with_pos, val_feat_with_pos)
                val_feat_with_pos = (val_feat_with_pos[:,:c] + val_feat_with_pos_DW)*0.5
                val_feat_with_pos = val_feat_with_pos.view(b, num_agents, num_agents, c, h, w).contiguous()

                val_feats[0] = val_feat_with_pos
            
            if prev_quality_maps is None:
                val_feats[0], communication_mask, communication_rate = _communication_graph_learning(val_feats[0], quality_maps[0], confidence_maps, round_id, thre=comm_thre)
            else:
                val_feats[0], communication_mask, communication_rate = _communication_graph_learning(val_feats[0], prev_quality_maps[0], confidence_maps, round_id, thre=comm_thre)
            val_feats[0], comp_gt, comp_out = _message_packing(val_feats[0], prev_quality_maps, confidence_maps, compress_flag=compress_flag)
            weight_mats = []
            for i, c_layer in enumerate(self.trans_layer):
                feat_map = x[c_layer]
                ...
                elif fusion == 'egodcntransformer':
                    feat_fuse, weight_mat = _transformer_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'difftransformer':
                    feat_fuse, weight_mat = _difftransformer_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'dcntransformer':
                    feat_fuse, weight_mat = _transformer_message_fusing(feat_map, val_feat, quality_map)
                    feat_fuse = feat_fuse.flatten(0,1)
                    feat_fuse_DW = eval('self.DC'+str(c_layer))(feat_fuse, return_offset=False)
                    feat_fuse = (feat_fuse + feat_fuse_DW)*0.5
                    feat_fuse = feat_fuse.view(b, num_agents, c, h, w).contiguous()
                elif fusion == 'cattransformer':
                    feat_fuse, weight_mat = _cattransformer_message_fusing(feat_map, val_feat, quality_map, confidence_maps)
                elif fusion == 'crosstransformer':
                    feat_fuse, weight_mat = _crosstransformer_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'weighttransformer':
                    feat_fuse, weight_mat = _weighttransformer_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'crossselftransformer':
                    feat_fuse, weight_mat = _crossselftransformer_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'selfcrosstransformer':
                    feat_fuse, weight_mat = _selfcrosstransformer_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'rescrosstransformer':
                    feat_b1, feat_fuse, weight_mat = _rescrosstransformer_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'cascrosstransformer':
                    feat_fuse, weight_mat = _cascrosstransformer_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'bevtransformer':
                    feat_fuse, weight_mat = _bevtransformer_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'noquery_maxcontext':
                    feat_fuse, weight_mat = _noquery_maxcontext_message_fusing(feat_map, val_feat, quality_map)
                elif fusion == 'query_maxcontext':
                    feat_fuse, weight_mat = _query_maxcontext_message_fusing(feat_map, val_feat, quality_map)
                else:
                    feat_fuse, weight_mat = _message_fusing(feat_map, val_feat, quality_map)
                weight_mats.append(weight_mat)
                x[c_layer] = feat_fuse
            weight_mats_dict[round_id] = weight_mats
            prev_quality_maps = quality_maps
    ...
    def GlobalCoord_forward(self, images, trans_mats, shift_mats, map_scale):
        ...
        if self.trans_layer[-1] == -2:
            ...
            if self.message_mode in ['Mean', 'Max', 'Pointwise']:
                ...
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, 'context', self.compress_flag)
            elif self.message_mode in ['QualityMapTransformer']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='transformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerDCN']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='dcntransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerEgoDCN']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='egodcntransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerDiff']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='difftransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerCat']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='cattransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerCross']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='crosstransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerWeight']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='weighttransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerSelfCross']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='selfcrosstransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerResCross']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='rescrosstransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerCasCross']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='cascrosstransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerCrossSelf']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='crossselftransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerBEV']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, fusion='bevtransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapNoQueryMaxContext']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, 'noquery_maxcontext', self.compress_flag)
            elif self.message_mode in ['QualityMapQueryMaxContext']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, communication_rate, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, self.comm_thre, 'query_maxcontext', self.compress_flag)
        ...
        if self.vis:
            ...
            for layer_ind, c_layer in enumerate(self.trans_layer):
                ...
                cur_time = '{}'.format(time.time())[-4:]
                for i in range(b):
                    save_dir = os.path.join(root_dir, 'qualitymap_attn_weights_mask', '{}_{}'.format(cur_time, i))
                    if not os.path.exists(save_dir):
                        os.makedirs(save_dir)
                    for j in range(q_agents):
        ...
        global_z['comm_rate'] = communication_rate
        return [global_z]
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4, message_mode='NO_MESSAGE_NOWARP', trans_layer=[3], coord='Local', warp_mode='HW', depth_mode='Unique', feat_mode='inter', feat_shape=[192, 352], round=1, compress_flag=False, comm_thre=0.1):
