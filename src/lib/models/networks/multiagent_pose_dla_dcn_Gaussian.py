...
class GumbelNoise(nn.Module):
    ...
    def forward(self, input_):
        ...
        if self.noise_type == 'new_U_B':
            ...
            raise ValueError('Unknown noise_type', self.noise_type)
        self.noise = self.noise.detach().to(input_.device)
        return (input_ + self.noise) * (1/self.t)
...
class DLASeg(nn.Module):
        ...
        if self.message_mode in ['When2com']:
            ...
            for c_layer in self.trans_layer:
                if c_layer >= 0:
                    ...
                    # Gaussian smoothing
                    # kernel_size = 5
                    # for sigma in [1, 0.8, 0.5]:
                    #     gaussian_filter = nn.Conv2d(1, 1, kernel_size=kernel_size, stride=1, padding=(kernel_size-1)//2)
                    #     self.init_gaussian_filter(gaussian_filter, kernel_size, sigma)
                    #     gaussian_filter.requires_grad = False
                    #     self.__setattr__('gaussian_filter'+str(int(10*sigma)), gaussian_filter)
                    self.noise = GumbelNoise(p=0.2, t=0.9, noise_type='new_U_B')
                    self.alpha = torch.nn.Parameter(torch.tensor(4.0))
    ...
    def QUALITYMAP_MESSAGE(self, x, shift_mats, round, comm_thre=0.1, fusion='context', compress_flag=False):
        ...
        def _transformer_message_fusing(query_feat, val_feat, quality_map, round_id=0):
            ...
            src2, weight_mat = eval('self.cross_attn'+str(round_id))(src, val, value=val, attn_mask=None, key_padding_mask=None)
            ...
            src = src + eval('self.dropout1_'+str(round_id))(src2)
            src = eval('self.norm1_'+str(round_id))(src)
            src2 = eval('self.linear2_'+str(round_id))(eval('self.dropout0_'+str(round_id))(F.relu(eval('self.linear1_'+str(round_id))(src))))
            src = src + eval('self.dropout2_'+str(round_id))(src2)
            src = eval('self.norm2_'+str(round_id))(src)
        ...
        def _get_topk_accord_B(comm_maps, conf_maps, B, round_id=0):
            b, q_agents, c, h, w = conf_maps.shape
            b, k_agents, q_agents, c, h, w = comm_maps.shape
            if round_id == 0:
                conf_maps = conf_maps.flatten(1)    # (b, q_agents*h*w)
                k = int(B * h * w * q_agents)
                topk_v, indices = torch.topk(conf_maps, k, dim=-1)  # topk_v: (b,max_len)
                
                ones_mask = torch.ones_like(conf_maps).to(comm_maps.device)
                zeros_mask = torch.zeros_like(conf_maps).to(comm_maps.device)
                comm_maps = comm_maps.flatten(2)  # (b, k_agents, q_agents*h*w)
                comm_mask = []
                for i in range(q_agents):
                    cur_comm_mask = torch.where((comm_maps[:,i]-topk_v[:,-1])>=1e-6, ones_mask, zeros_mask)
                    comm_mask.append(cur_comm_mask.unsqueeze(1))
                comm_mask = torch.cat(comm_mask, dim=1) # (b, k_agents, q_agents*h*w)
                comm_mask = comm_mask.view(b, k_agents, q_agents, c, h, w)
            
            else:
                b, k_agents, q_agents, c, h, w = comm_maps.shape
                comm_maps = comm_maps.flatten(1)  # (b, q_agents*k_agents*h*w)
                k = int(B * h * w * q_agents * k_agents)
            
                topk_v, indices = torch.topk(comm_maps, k, dim=-1)  # topk_v: (b*num_agents*num_agents,max_len)

                ones_mask = torch.ones_like(comm_maps).to(comm_maps.device)
                zeros_mask = torch.zeros_like(comm_maps).to(comm_maps.device)
                comm_mask = zeros_mask.scatter_(1, indices, ones_mask)
                comm_mask = comm_mask.view(b, k_agents, q_agents, c, h, w)
            return comm_mask
        ...
        def _learn_gaussian_filter(quality_maps, B, k_size=5, sigma=None):
            b, k_agents, q_agents, c, h, w = quality_maps.shape
            quality_maps = quality_maps.view(b*k_agents*q_agents, c, h, w)

            if sigma is None:
                sigma = self.alpha * torch.log(torch.tensor(B)+1e-6).to(quality_maps.device) + self.beta
                sigma = sigma.sigmoid()
            else:
                sigma = torch.tensor(sigma).to(quality_maps.device)
            # print('B: {}, alpha:{}, beta:{}, sigma: {}'.format(B, self.alpha.item(), self.beta.item(), sigma.item()))
            # import ipdb; ipdb.set_trace()

            center = k_size // 2
            xs = torch.linspace(0 - center, k_size - center - 1, steps=k_size)
            ys = torch.linspace(0 - center, k_size - center - 1, steps=k_size)
            x, y = torch.meshgrid(xs, ys)
            x = x.to(quality_maps.device)
            y = y.to(quality_maps.device)
            filters = 1 / (2 * torch.tensor(np.pi).to(quality_maps.device) * sigma) * torch.exp(-(torch.square(x) + torch.square(y)) / (2 * torch.square(sigma)))
            filters = filters.unsqueeze(0).unsqueeze(0)

            return filters
        
        def _smooth_comm_map(comm_maps, filters):
            len_shape = len(comm_maps.shape)
            if len_shape == 6:
                b, k_agents, q_agents, c, h, w = comm_maps.shape
                comm_maps = comm_maps.flatten(0,2)
            else:
                b, q_agents, c, h, w = comm_maps.shape
                comm_maps = comm_maps.flatten(0,1)
            k_size = filters.shape[-1]
            comm_maps = F.conv2d(comm_maps, filters, padding=(k_size-1)//2)
            if len_shape == 6:
                comm_maps = comm_maps.view(b, k_agents, q_agents, c, h, w)
            else:
                comm_maps = comm_maps.view(b, q_agents, c, h, w)
            return comm_maps

        def _communication_graph_learning(val_feats, quality_maps, confidence_maps, round_id=0, thre=1.0, mask_on_feat=False):
            # val_feats: (b, k_agents, q_agents, c, h, w) 
            ...
            k_size = 5
            B_thres = [1, 0.113694, 0.014132, 0.003331, 0.001488, 0.001155, 0.000981, 0.000830, 0.000735, 0.000646, 0.000579, 0.000523, 0.000499, 0.000397, 0.000312, 0.000232, 0.000002, 0]
            if round_id == 0:
                B = thre
            else:
                B = thre
            if B == 0.0:
                ...
                # filters = _learn_gaussian_filter(communication_maps, B, k_size)
                # communication_maps = _smooth_comm_map(communication_maps, filters)
                # confidence_maps = _smooth_comm_map(confidence_maps, filters)
                # score_map = self.noise(score_map)
                # score_map = score_map.sigmoid()
                communication_mask = _get_topk_accord_B(communication_maps, confidence_maps, B, round_id)
            
            communication_rate = torch.tensor(B).to(communication_maps.device)
            communication_maps_topk = communication_mask * communication_maps

            # thre = np.random.choice([0.13, 0.16, 0.20, 0.24, 0.28, 1.0])
            ...
            # B_thre = [1, 0.113694, 0.014132, 0.003331, 0.001488, 0.001155, 0.000981, 0.000830, 0.000735, 0.000646, 0.000579, 0.000523, 0.000499, 0.000397, 0.000312, 0.000232, 0.000002, 0]
            # B_dict = OrderedDict()
            # for thre_id, c_thre in enumerate(comm_thre):
            #     B_dict[c_thre] = B_thre[thre_id]
            # communication_mask, communication_maps_topk, filters = _learn_gaussian_filter(communication_maps, B=B_dict[thre], k_size=5)
            # communication_mask = torch.where((communication_maps_topk - thre)>1e-6, ones_mask, zeros_mask)
            ...
            #     confidence_maps = F.conv2d(confidence_maps, filters, padding=(5-1)//2)
            #     confidence_maps = confidence_maps.view(b, q_agents, 1, h, w)
            ...
            if not self.training:
                ...
                if mask_on_feat:
                    val_feats_aftercom = val_feats * communication_maps_topk_nodiag
                else:
                    val_feats_aftercom = val_feats * communication_mask_nodiag
        ...
        for round_id in range(round):
            ...
            mask_on_feat = True if fusion == 'transformer' else False
            if prev_confidence_maps is None:
                val_feats[0], communication_mask, communication_rate, quality_maps[0] = _communication_graph_learning(val_feats[0], quality_maps[0], confidence_maps, round_id, thre=comm_thre, mask_on_feat=mask_on_feat)
            else:
                val_feats[0], communication_mask, communication_rate, quality_maps[0] = _communication_graph_learning(val_feats[0], quality_maps[0], prev_confidence_maps, round_id, thre=comm_thre, mask_on_feat=mask_on_feat)
