...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
        ...
        self.sft_mode = True if len(message_mode.split('+')) > 1 else False
        ...
        if self.sft_mode:
            for c_layer in self.trans_layer:
                if c_layer >= 0:
                    in_channels = 64*2**c_layer
                    sft = SFT(in_channels, 1)
                    self.__setattr__('sft'+str(c_layer), sft)
    ...
    def QUALITYMAP_MESSAGE(self, x, shift_mats, round=1, fusion='context', compress_flag=False):
        ...
        def _crosstransformer_message_fusing(query_feat, val_feat, quality_map, grid=25, max_len=200, local_len=200, bin=False):
            ...
            h_indices = (flatten_indices // w).unsqueeze(1)
            w_indices = (flatten_indices % w).unsqueeze(1)
            p_dis = dis_map(h, w, h_indices, w_indices, grid=grid, bin=bin) # (b*num_agents*max_len, h, w)
        ...
        def _rescrosstransformer_message_fusing(query_feat, val_feat, quality_map):
            ...
            top_feat_fuse, top_weight_mat = _crosstransformer_message_fusing(feat_fuse, val_feat, quality_map, grid=10, max_len=100, local_len=100, bin=True)
            ...
            final_feat_fuse = top_feat_fuse + feat_fuse
        ...
        for round_id in range(1):
            ...
            if self.sft_mode:
                init_x = x[self.trans_layer[0]]
                init_x = init_x.flatten(0,1)
                q_maps = confidence_maps.flatten(0,1)
                sft_x = self.__getattr__('sft'+str(self.trans_layer[0]))(init_x, q_maps)
                x[self.trans_layer[0]] = sft_x.view(b, num_agents, sft_x.shape[-3], sft_x.shape[-2], sft_x.shape[-1])
            val_feats = self.get_colla_feats(x, shift_mats, self.trans_layer, with_pos=True) # (b, k_agents, q_agents, c, h, w)
            ...
            # val_feats[0], communication_mask = _communication_graph_learning(val_feats[0], quality_maps[0], confidence_maps, thre=0.00)
    ...
    def GlobalCoord_forward(self, images, trans_mats, shift_mats, map_scale):
        ...
        if self.trans_layer[-1] == -2:
            ...
            if self.message_mode in ['Mean', 'Max', 'Pointwise']:
                ...
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, fusion='cattransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerCross']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, fusion='crosstransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerWeight']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, fusion='weighttransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerSelfCross']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, fusion='selfcrosstransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerResCross']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, fusion='rescrosstransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerCasCross']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, fusion='cascrosstransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerCrossSelf']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, fusion='crossselftransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapTransformerBEV']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, fusion='bevtransformer', compress_flag=self.compress_flag)
            elif self.message_mode in ['QualityMapNoQueryMaxContext']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, 'noquery_maxcontext', self.compress_flag)
            elif self.message_mode in ['QualityMapQueryMaxContext']:
                colla_x, weight_mats, results, quality_map, val_feat, communication_mask, comp_gt, comp_out = self.QUALITYMAP_MESSAGE(single_x, shift_mats, self.round, 'query_maxcontext', self.compress_flag)
