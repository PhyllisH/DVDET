...
class DLASeg(nn.Module):
    ...
    def QUALITYMAP_MESSAGE(self, x, shift_mats, round, comm_thre=0.1, fusion='context', compress_flag=False):
        ...
        def _communication_graph_learning(val_feats, quality_maps, confidence_maps, prev_communication_mask, round_id=0, thre=0.03):
            # val_feats: (b, k_agents, q_agents, c, h, w) 
        ...
        for round_id in range(round):
            ...
            if prev_confidence_maps is None:
                val_feats[0], communication_mask, communication_rate, _ = _communication_graph_learning(val_feats[0], quality_maps[0], confidence_maps, prev_comm_mask, round_id, thre=comm_thre)
            else:
                val_feats[0], communication_mask, communication_rate, _ = _communication_graph_learning(val_feats[0], quality_maps[0], prev_confidence_maps, prev_comm_mask, round_id, thre=comm_thre)
            ...
            for i, c_layer in enumerate(self.trans_layer):
                ...
                if round_id >= 2:
                    ...
                    for i in range(num_agents):
                        communication_mask_list.append(torch.cat([communication_mask[:,:i,i],communication_mask[:,i+1:,i]], dim=1).unsqueeze(2))
                    communication_mask_list = torch.cat(communication_mask_list, dim=2) # (1, 4, 5, 1, h, w)
                    communication_mask_max = communication_mask_list.max(dim=1)[0]
                    x[c_layer] = feat_fuse * communication_mask_max + x[c_layer] * (1-communication_mask_max)
                else:
                    x[c_layer] = feat_fuse
            ...
            if round_id ==0:
                if comm_thre==0.0:
                    communication_rates = torch.ones_like(communication_rate).to(communication_rate.device)
                else:
                    communication_rates = communication_rate
            else:
                communication_rates += communication_rate
        ...
        return x, weight_mat, results_dict, quality_map, val_feat, communication_mask, communication_rates, comp_gt, comp_out
